...
help:
	...
	@echo "  up            Start the full stack (HDFS + Spark + Kafka + Airflow + Pinot + Superset + Flink + HMS)"
	@echo "  up-core       Start only HDFS + Spark + Kafka (skip Airflow / Pinot / Superset / Flink / HMS)"
	@echo "  up-bi         Start only Pinot + Superset (skip everything else)"
	@echo "  up-stream     Start only Kafka + Flink (skip everything else)"
	@echo "  up-dwh        Start only the Hive Metastore stack (metastore-db + hive-metastore)"
	@echo "  down          Stop containers (keep volumes)"
	@echo "  stop          Same as down"
	@echo "  nuke          Stop and DELETE all volumes (HDFS, Kafka, Postgres, Pinot, Superset, Flink, HMS data)"
